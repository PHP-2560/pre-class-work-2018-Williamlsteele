...
gm %>%
  group_by(country) %>%
  mutate(pop_diff = pop - lag(pop), lifeExp_diff = lifeExp - lag(lifeExp)) %>% #creating two new variables that calculate the difference between the most recent population/lifeexpectancy and the one "lagging" one observation before it
  filter(pop_diff < 0 & lifeExp_diff > 0) %>% #filtering observations where the population descreased and the life expectancy increased
  select(-pop_diff, -lifeExp_diff) #returning the original dataset where the two new variables are not shown
```
