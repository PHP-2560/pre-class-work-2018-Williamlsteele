...
``` {r}
sum_no_loop = sum(seq(0, 1000, by = 1))
sum_no_loop

sum_no_loop_even = sum(seq(0,1000, by = 2))
sum_no_loop_even
```
Question 3:
...
for(row in 1:nrow(survey)) {
  for(col in 2:ncol(survey)) {
    if(survey.clean[row,col] < 0 | survey.clean[row,col] > 5 )
...
for(row in 1:6) {
  counter = 0
  for(col in 2:6) {
    if(is.na(survey.clean[row,col])) {
      counter = counter+1
    }
  }
  survey.clean$invalid.answers[row] = counter
}
survey.clean 
